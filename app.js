...
function mergeGlobal(remote){
  ...
  const domains=[];
  const playersChanged=remote.players!==undefined?mergePlayersRemote(remote.players,remote.playerTombstones):false;
  if(playersChanged)domains.push('players');
...
function save(){markLocalPlayerChanges();storageSet(KEY,JSON.stringify(state));storageSet('a_profecia_local_changed_at',String(Date.now()));queueRemoteSave();saveLocalRecovery('save').catch(()=>{})}
async function saveGlobalNow(){markLocalPlayerChanges();storageSet(KEY,JSON.stringify(state));storageSet('a_profecia_local_changed_at',String(Date.now()));saveLocalRecovery('global-save').catch(()=>{});if(remoteEnabled&&remoteHydrated&&!remoteApplying){try{const old=await fetchGlobal();if(old?.data)await backupGlobal(old.data,'antes-da-escrita');}catch(e){console.warn('Não foi possível criar backup remoto:',e)}const result=await pushGlobal(globalPayload());return result}return null}
