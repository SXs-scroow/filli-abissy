import { remoteEnabled, fetchGlobal, pushGlobal, uploadGlobalFile, subscribeGlobal, subscribeLive, broadcastLive } from './src/globalSync.js';
...
function mergeGlobal(remote){
  ...
  const domains=[];
  for(const key of ['siteBrand','players','creatures','items','spells','uiIcons','backgrounds','music','slasherIntroMusic','characterRules','classBonuses','customContent','sessionBoard','terrorMode','nexus','tvScreen','tvScenes','sounds','soundboard']){
...
function hasRemoteSharedData(remote){
  return !!(remote&&typeof remote==='object'&&['siteBrand','players','creatures','items','spells','uiIcons','backgrounds','music','slasherIntroMusic','characterRules','classBonuses','customContent','sessionBoard','terrorMode','nexus','tvScreen','tvScenes','sounds','soundboard'].some(k=>remote[k]!==undefined));
}
...
function playerSheetFallback(error){
  ...
  <article class="sheet-card"><div class="section-heading"><div><span class="section-kicker">ATRIBUTOS</span><h2>Distribuição de Atributos</h2></div><strong id="attrRemaining">Restam ${allocationRemaining(p).attrs}</strong></div><div class="attribute-grid">${attrs.map(a=>`<label class="attribute-box"><span class="attr-icon">${uiAttrIcon(a)}</span><span class="attr-name">${esc(a)}</span><input class="pAttr" data-attr="${esc(a)}" type="number" min="0" max="8" value="${p.attrs?.[a]||0}"><span class="attr-cap">/ 8</span></label>`).join('')}</div><button class="btn primary" id="saveAttrs">Salvar atributos</button></article>
  <article class="sheet-card"><div class="section-heading"><div><span class="section-kicker">PERÍCIAS</span><h2>Distribuição de Perícias</h2></div><strong id="skillRemaining">Restam ${allocationRemaining(p).skills}</strong></div><div class="skills-groups"><div class="skill-group">${skills.map(sk=>`<label class="skill-line"><span class="skill-name"><i class="skill-icon">${uiSkillIcon(sk)}</i><span>${esc(sk)}</span></span><input class="pSkill" data-skill="${esc(sk)}" type="number" min="0" max="15" value="${p.skills?.[sk]||0}"><b>/15</b></label>`).join('')}</div></div><button class="btn primary" id="saveSkills">Salvar perícias</button></article>
...
function admin(){return shell(`<section class="hero"><span class="badge">MESTRE</span><h1>Câmara do Mestre</h1><p>Gerenciamento completo de Players, Monstros, classes, itens e trilha.</p></section><div class="panel-tabs"><button class="active" data-tab="players">Players</button><button data-tab="monsters">Monstros</button><button data-tab="classes">Classes</button><button data-tab="items">Banco de Itens</button><button data-tab="music">Trilha global</button><button data-tab="sounds">Soundboard</button><button data-tab="secrets">Pistas secretas</button><button data-tab="symbols">Símbolos</button><button data-tab="spells">Magias</button><button data-tab="content">Conteúdo</button><button data-tab="backgrounds">Fundos</button><button data-tab="branding">Identidade</button><button data-tab="tv">Tela da TV</button></div><div id="adminContent">${playersAdmin()}</div>`,'master')}
  ...
  try{
    if(remoteEnabled){
      ...
      const applyIncoming=async(row)=>{
        ...
        if(d.some(x=>!['music','backgrounds'].includes(x))){if(currentView==='tv'&&d.every(x=>x==='tvScreen'||x==='tvScenes'))applyTvCommand(tvCurrent(),true);else render(currentView||'home');}
