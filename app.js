...
function skipToNextPersonalTheme(currentIndex){
  ...
  const audioThemes=p.musicThemes.filter((t,i)=>themeIsAudio(t));
  if(audioThemes.length===0)return;
  let nextIdx=currentIndex+1;
  if(nextIdx>=audioThemes.length)nextIdx=0;
  const nextTheme=audioThemes[nextIdx];
...
function renderPersonalPlayer(title,index){
  ...
  root.querySelector('#ppRewind').onclick=()=>{if(personalAudio){personalAudio.currentTime=0}};
  root.querySelector('#ppNext').onclick=()=>skipToNextPersonalTheme(index);
  root.querySelector('#ppVolume').oninput=e=>{const vol=Math.max(0,Math.min(1,Number(e.target.value)/100));if(personalAudio)personalAudio.volume=vol;root.querySelector('#ppVolumeValue').textContent=Math.round(Number(e.target.value))+'%'};
