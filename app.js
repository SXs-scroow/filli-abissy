...
const DEFAULT={version:31,session:null,characterRules:clone(DEFAULT_RULES),music:{type:'',url:'',title:'',kind:'url',mediaId:''},sounds:[],players:[basePlayer()],creatures:[],items:DEFAULT_ITEMS,spells:INITIAL_SPELLS,uiIcons:{attrs:{...DEFAULT_ATTR_ICONS},skills:{...DEFAULT_SKILL_ICONS},conditions:{}},backgrounds:{},customContent:{attrs:[],skills:[],conditions:[],deities:[]}};
let memoryStore={};
let bgObjectUrls={};
...
function load(){try{const raw=storageGet(KEY)||storageGet('filii_abyssi_state_v10')||storageGet('filii_abyssi_state_v8');const saved=raw?JSON.parse(raw):null;if(!saved)return clone(DEFAULT);saved.characterRules={campaign:{...DEFAULT_RULES.campaign,...(saved.characterRules?.campaign||{})},slasher:{...DEFAULT_RULES.slasher,...(saved.characterRules?.slasher||{})}};saved.items=migrateItems(saved.items);saved.players=Array.isArray(saved.players)?saved.players:[];if(!saved.players.some(p=>p.login===TEST.login))saved.players.push(basePlayer());saved.creatures=Array.isArray(saved.creatures)?saved.creatures:[];saved.sounds=Array.isArray(saved.sounds)?saved.sounds:[];saved.spells=Array.isArray(saved.spells)&&saved.spells.length?saved.spells:INITIAL_SPELLS.map(clone);saved.uiIcons=saved.uiIcons||{attrs:{},skills:{}};saved.uiIcons.attrs={...DEFAULT_ATTR_ICONS,...(saved.uiIcons.attrs||{})};saved.uiIcons.skills={...DEFAULT_SKILL_ICONS,...(saved.uiIcons.skills||{})};saved.uiIcons.conditions={...(saved.uiIcons.conditions||{})};saved.backgrounds={...(saved.backgrounds||{})};saved.customContent={attrs:[],skills:[],conditions:[],deities:[],...(saved.customContent||{})};if(typeof saved.music==='string')saved.music={type:'audio',url:saved.music,title:'Trilha da sessão'};saved.music=saved.music||{type:'',url:'',title:'',kind:'url',mediaId:''};if(typeof saved.music==='object'){saved.music.kind=saved.music.kind||'url';saved.music.mediaId=saved.music.mediaId||''}saved.players.forEach(p=>{p.musicThemes=Array.isArray(p.musicThemes)?p.musicThemes:[];p.homeWallpaper=p.homeWallpaper||'';normalize(p)});saved.creatures.forEach(normalize);saved.version=30;return saved}catch{return clone(DEFAULT)}}
let remoteSaveTimer=null,remoteHydrated=false,remoteApplying=false;
function globalPayload(){
  const {session,sounds,...shared}=state;
  // Música da campanha também é global: URL, estado, loop e posição são
  // compartilhados. Sons soltos continuam locais para não inflar o estado.
  return {version:shared.version,players:shared.players,creatures:shared.creatures,items:shared.items,spells:shared.spells,uiIcons:shared.uiIcons,backgrounds:shared.backgrounds,music:shared.music,characterRules:shared.characterRules,customContent:shared.customContent};
}
...
function mergeGlobal(remote){
  ...
  const domains=[];
  for(const key of ['players','creatures','items','spells','uiIcons','backgrounds','music','characterRules','customContent']){
  ...
  if(domains.length){
    ...
    state.backgrounds={...(state.backgrounds||{})};state.customContent={attrs:[],skills:[],conditions:[],deities:[],...(state.customContent||{})};
    state.characterRules={campaign:{...DEFAULT_RULES.campaign,...(state.characterRules?.campaign||{})},slasher:{...DEFAULT_RULES.slasher,...(state.characterRules?.slasher||{})}};
...
function hasRemoteSharedData(remote){
  return !!(remote&&typeof remote==='object'&&['players','creatures','items','spells','uiIcons','backgrounds','music','characterRules','customContent'].some(k=>remote[k]!==undefined));
}
...
async function saveGlobalNow(){storageSet(KEY,JSON.stringify(state));if(remoteEnabled&&remoteHydrated&&!remoteApplying){const result=await pushGlobal(globalPayload());return result}return null}
...
function rollExpressionFor(p,expr){
 ...
 const roll={value,die:label,label,time:new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),rawRolls:rolls,modifier:mod,diceTotal};
 p.rolls=[...(p.rolls||[]),roll].slice(-30);save();updateDiceUI(roll);return roll;
}
function rollDiceFor(p,sides,label=null){
 ...
 const roll={value:n,label:label||`1d${die}`,die:`d${die}`,time:new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),rawRolls:[n],modifier:0,diceTotal:n};p.rolls=[...(p.rolls||[]),roll].slice(-30);save();updateDiceUI(roll);return roll;
}
...
function updateDiceUI(roll){
 ...
 if(history){const empty=history.querySelector('.empty');if(empty)empty.remove();const entry=document.createElement('div');entry.className='roll-entry roll-entry-new';entry.innerHTML=`<strong>${esc(rollDisplayText(roll))}</strong><span>${esc(roll.die||roll.label)}</span><time>${esc(roll.time)}</time>`;history.prepend(entry);while(history.children.length>12)history.lastElementChild?.remove()}
}
...
function addBag(p){
 const options=state.items.map(i=>`<option value="${esc(i.id)}">ID ${String(i.id).padStart(3,'0')} — ${esc(i.name)}</option>`).join('');
 openModal(`<div class="modal" id="bagAddModal"><div class="modal-card"><button class="modal-close" id="closeBagAdd">×</button><h2>Adicionar item à mochila</h2><p class="small muted">Capacidade atual: ${bagCapacity(p)} espaços.</p><div class="field"><label>Item</label><select id="bagItemSelect"><option value="">Escolha um item...</option>${options}</select></div><div class="field"><label>Quantidade</label><input id="bagItemQty" type="number" min="1" value="1"></div><div class="row" style="justify-content:flex-end"><button class="btn" id="cancelBagAdd">Cancelar</button><button class="btn primary" id="confirmBagAdd">Adicionar</button></div></div></div>`);
 const close=()=>document.getElementById('bagAddModal')?.remove();document.getElementById('closeBagAdd').onclick=close;document.getElementById('cancelBagAdd').onclick=close;
 document.getElementById('confirmBagAdd').onclick=()=>{const it=item(Number(document.getElementById('bagItemSelect').value));if(!it){toast('Escolha um item.');return}const qty=Math.max(1,Number(document.getElementById('bagItemQty').value)||1),weight=inferWeight(it),used=bagWeight(p),cap=bagCapacity(p),ex=p.backpack.find(x=>Number(x.id)===Number(it.id)),current=ex?.qty||0;const maxByWeight=Math.floor((cap-used+weight*current)/weight),add=Math.min(it.maxQty-current,qty,maxByWeight);if(add<=0){toast('Não há espaço suficiente.');return}if(ex)ex.qty=current+add;else p.backpack.push({id:it.id,qty:add});save();close();toast(`${add} × ${it.name} adicionado à mochila.`);render('sheet')};
}
function slots(e,editable=true){const used=bagWeight(e);return `<div class="row space"><div><h3>Mochila</h3><span class="small ${used>MAX_BAG?'danger-text':'muted'}">${used}/${bagCapacity(e)} espaços usados • peso por unidade: 1–3</span></div>${editable?'<button class="btn small" id="addBagItem">Adicionar item</button>':'<span class="small muted">Somente o Mestre pode alterar</span>'}</div><div class="slot-grid" style="margin-top:12px">${Array.from({length:bagCapacity(e)},(_,i)=>{const b=e.backpack[i],it=b&&item(b.id);return `<div class="slot ${it?'filled':''}">${it?`${editable?`<button data-remove-slot="${i}">×</button>`:''}<div class="slot-icon">${itemIcon(it)}</div><div class="slot-name">${esc(it.name)}</div>${String(it.category||'').toLowerCase().includes('livro')||it.bookContent?`<button class="book-read-btn" data-read-book="${it.id}">Ler</button>`:''}<div class="slot-id">ID ${String(it.id).padStart(3,'0')} • Peso ${inferWeight(it)}</div><div class="slot-qty">Qtd. ${b.qty} • ${inferWeight(it)*b.qty} espaço${inferWeight(it)*b.qty===1?'':'s'}</div>`:'<div class="slot-icon" style="opacity:.15">◇</div><div class="small muted">Espaço vazio</div>'}</div>`}).join('')}</div>`}
function spellMarkup(p){
 if(p.class==='Ocultista'){
  const gods=allDeities(),chosen= gods.find(d=>d.id===p.deityId);
  if(chosen)return `<article class="sheet-card deity-card selected-deity"><div class="section-heading"><div><span class="section-kicker">AFINIDADE</span><h2>${esc(chosen.name)}</h2></div><span class="badge">DEUS ESCOLHIDO</span></div>${chosen.image?`<img class="deity-choice-image" src="${esc(chosen.image)}" alt="">`:''}<p class="muted">Sua afinidade foi selada. O símbolo escolhido permanece junto à Alma.</p></article>`;
  if(!gods.length)return `<article class="sheet-card deity-card"><div class="section-heading"><div><span class="section-kicker">AFINIDADE</span><h2>Aguardando os Deuses</h2></div></div><p class="muted">O Mestre ainda não cadastrou os Deuses disponíveis para Ocultistas.</p></article>`;
  return `<article class="sheet-card deity-card"><div class="section-heading"><div><span class="section-kicker">AFINIDADE</span><h2>Escolha um Deus</h2></div><span class="corner-mark">1 escolha</span></div><p class="muted small">Você começa com Afinidade. Escolha uma entidade; depois desta escolha, esta área desaparece.</p><div class="deity-grid">${gods.map(d=>`<button class="deity-option" type="button" data-deity-choice="${esc(d.id)}">${d.image?`<img src="${esc(d.image)}" alt="">`:'<span>◇</span>'}<strong>${esc(d.name)}</strong></button>`).join('')}</div></article>`;
 }
 if(!hasMagicClass(p.class))return '';
 const spells=getSpells(),selected=spells.find(x=>x.id===p.initialSpell);if(selected)return `<article class="sheet-card spell-card selected-spell"><div class="section-heading"><div><span class="section-kicker">MAGIA INICIAL</span><h2>${esc(selected.name)}</h2></div><span class="dice-type">${esc(selected.cost)}</span></div>${selected.image?`<img class="spell-image" src="${esc(selected.image)}" alt="">`:''}<p class="spell-school">${esc(selected.school)}</p><p>${esc(selected.description)}</p><div class="row"><span class="badge">ESCOLHIDA</span></div></article>`;return `<article class="sheet-card spell-card"><div class="section-heading"><div><span class="section-kicker">MAGIAS INICIAIS</span><h2>Escolha apenas uma</h2></div><span class="corner-mark">1 escolha</span></div><p class="muted small">As demais desaparecem assim que você escolher uma. As imagens são definidas pelo Mestre.</p><div class="spell-grid">${spells.map(sp=>`<button type="button" class="spell-option" data-spell-choice="${esc(sp.id)}">${sp.image?`<img src="${esc(sp.image)}" alt="">`:`<span class="spell-placeholder">✦</span>`}<strong>${esc(sp.name)}</strong><small>${esc(sp.school)}</small><span>${esc(sp.cost)}</span></button>`).join('')}</div></article>`}
function sheetInternal(){const p=player();if(!p)return shell('<div class="empty">Player não encontrado.</div>','sheet');syncDerivedResources(p);const last=p.rolls?.[p.rolls.length-1];const hpMax=derivedMax(p,'Corpo')||1;const sanityMax=derivedMax(p,'Sanidade');const bonus=classBonusMap(p.class);const chosen=classChosen(p);const currentClass=CLASSES[p.class]||{desc:'Escolha uma classe para revelar suas habilidades e bônus.',bonus:'—',abilities:[]};return shell(`<section class="profile-header"><div><span class="badge">PLAYER</span><h1>${esc(p.name)}</h1><p>${esc(p.class||'Classe não escolhida')} <span class="sep">•</span> ${esc(p.status)}</p><div class="destiny-phrase" id="destinyPhrase">Seu destino está selado</div></div><div class="profile-seal"><img src="runa-gold.png" alt=""></div></section><div class="sheet-layout"><section class="sheet-main"><article class="sheet-card character-card"><div class="character-portrait"><img class="photo" id="charPhoto" src="${p.photo||'ritual.webp'}" alt="Retrato"><label class="upload-label">Alterar retrato<input id="photoInput" type="file" accept="image/png,image/jpeg,image/webp,.jfif" hidden></label></div><div class="character-info"><div class="eyebrow">IDENTIDADE</div><div class="field-inline"><label>Nome</label><input id="pName" value="${esc(p.name)}"></div><div class="field-inline"><label>Classe</label><div class="class-lock-field">${chosen?`<strong>${esc(p.class)}</strong><span>Escolha já selada</span>`:`<select id="pClass"><option value="">Escolha sua classe</option>${Object.keys(CLASSES).map(c=>`<option ${p.class===c?'selected':''}>${esc(c)}</option>`).join('')}</select>`}</div></div><div class="small-stat-row"><div><span>PONTOS EXTRAS</span><strong>${p.extra||0}</strong></div><div><span>VIDA MÁX.</span><strong>${hpMax}</strong></div><div><span>SANIDADE MÁX.</span><strong>${sanityMax}</strong></div></div><div class="class-bonus-summary"><span>BÔNUS DA CLASSE</span><strong>${esc(classBonusText(p.class))}</strong></div><button class="btn primary" id="saveIdentity">Salvar ficha</button></div></article>${playerThemesMarkup(p)}${playerWallpaperMarkup(p)}<article class="sheet-card resources-card"><div class="section-heading"><div><span class="section-kicker">RECURSOS</span><h2>Estado do corpo e da mente</h2></div></div>${hpFields(p,true)}</article><article class="sheet-card"><div class="section-heading"><div><span class="section-kicker">ATRIBUTOS</span><h2>Distribuição de Atributos</h2></div><span class="corner-mark">${Number(characterRules(p.campaignType).attrPoints)+(Number(p.attrBonusPoints)||0)} pontos totais • <b id="attrRemaining">Restam ${allocationRemaining(p).attrs}</b></span></div><div class="attribute-grid">${allAttrs().map(a=>{const b=bonus[a]||0;return `<label class="attribute-box"><span class="attr-icon">${uiAttrIcon(a)}</span><span class="attr-name">${esc(a)}</span><input class="pAttr" data-attr="${esc(a)}" type="number" min="0" max="8" value="${p.attrs[a]}" ><span class="attr-cap">/ 8</span>${b?`<small class="attr-bonus">+${b} classe • efetivo ${effectiveAttr(p,a)}</small>`:''}</label>`}).join('')}</div><p class="resource-rule">Corpo e Sanidade determinam automaticamente os limites de Vida e Sanidade. Máximo de 35.</p><button class="btn primary" id="saveAttrs">Salvar atributos</button></article><article class="sheet-card"><div class="section-heading"><div><span class="section-kicker">PERÍCIAS</span><h2>Distribuição de Perícias</h2><p class="small muted">Distribuídos: ${allocationTotals(p).skills} / ${characterRules(p.campaignType).skillPoints} • <strong id="skillRemaining">Restam ${allocationRemaining(p).skills}</strong></p></div><span class="corner-mark">${Number(characterRules(p.campaignType).skillPoints)+(Number(p.skillBonusPoints)||0)} pontos totais</span></div><div class="skills-groups">${Object.entries({...SKILL_GROUPS,'Personalizadas':customList('skills').map(x=>x.name).filter(Boolean)}).map(([g,items])=>`<div class="skill-group"><div class="skill-group-title">${esc(g)}</div>${items.map(sk=>`<label class="skill-line"><span class="skill-name"><i class="skill-icon">${uiSkillIcon(sk)}</i><span>${esc(sk)}${classBonusMap(p.class)[sk]?`<small class="skill-bonus">+${classBonusMap(p.class)[sk]} classe • efetivo ${effectiveSkill(p,sk)}</small>`:''}</span></span><input class="pSkill" data-skill="${esc(sk)}" type="number" min="0" max="15" value="${p.skills[sk]||0}"><b>/15</b></label>`).join('')}</div>`).join('')}</div><button class="btn primary" id="saveSkills">Salvar perícias</button></article><article class="sheet-card dice-section-fixed dice-v2">
...
function admin(){return shell(`<section class="hero"><span class="badge">MESTRE</span><h1>Câmara do Mestre</h1><p>Gerenciamento completo de Players, Monstros, classes, itens e trilha.</p></section><div class="panel-tabs"><button class="active" data-tab="players">Players</button><button data-tab="monsters">Monstros</button><button data-tab="classes">Classes</button><button data-tab="items">Banco de Itens</button><button data-tab="music">Trilha global</button><button data-tab="sounds">Sons</button><button data-tab="symbols">Símbolos</button><button data-tab="spells">Magias</button><button data-tab="content">Conteúdo</button><button data-tab="backgrounds">Fundos</button></div><div id="adminContent">${playersAdmin()}</div>`,'master')}
...
function itemModal(id=null){
 const e=id===null?{id:'',name:'',description:'',category:'Comum',maxQty:10,icon:'✦',effects:'',rarity:'Comum',bookPages:[]}:clone(item(id));
