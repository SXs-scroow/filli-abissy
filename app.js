...
function home(){
 ...
 const wallpaper=p?.homeWallpaper?` style="--player-wallpaper:url('${esc(p.homeWallpaper)}')"`:'';const themes=Array.isArray(p?.musicThemes)?p.musicThemes:[];return shell(`<section class="op-home-sheet player-personalized"${wallpaper}><div class="op-topline"><div><span class="badge">PERSONAGEM</span><h1>${esc(p?.name||'Player')}</h1><p>${esc(p?.class||'Classe não escolhida')} • ${esc(p?.status||'Ativo')}</p></div><div class="op-resource-strip horror-home-bars">${determinationBadge(p,true)}${resourceBarMarkup('Vida',p?.hp||0,derivedMax(p,'Corpo'),'health',false)}${resourceBarMarkup('Sanidade',p?.sanity||0,derivedMax(p,'Sanidade'),'sanity',false)}</div></div><div class="op-dossier-grid"><section class="op-attributes"><div class="section-heading"><div><span class="section-kicker">ATRIBUTOS</span><h2>Estado atual</h2></div><span class="corner-mark">SOMENTE LEITURA</span></div><div class="op-attr-list">${ATTRS.map(a=>`<div class="op-attr-row"><span class="op-attr-symbol">${uiAttrIcon(a)}</span><span>${esc(a)}</span><b>${effectiveAttr(p,a)}</b></div>`).join('')}</div></section><section class="op-trained"><div class="section-heading"><div><span class="section-kicker">PERÍCIAS</span><h2>Treinadas</h2></div><span class="corner-mark">${trained.length}</span></div><div class="op-trained-list">${trained.length?trained.map(sk=>`<div class="op-skill-row"><span>${uiSkillIcon(sk)} ${esc(sk)}</span><b>${effectiveSkill(p,sk)}</b></div>`).join(''):'<div class="empty">Nenhuma perícia treinada.</div>'}</div></section><aside class="op-profile"><div class="op-portrait-frame"><img src="${esc(p?.photo||'ritual.webp')}" alt="Retrato do personagem"><span class="portrait-mark">✦</span></div><div><span class="section-kicker">REGISTRO</span><h2>${esc(p?.class||'Sem classe')}</h2><p>Ficha inicial inspirada em uma folha de personagem: leitura rápida, atributos totais e apenas perícias treinadas.</p></div></aside></div><div class="op-home-actions"><button class="btn primary" data-nav="sheet">Abrir ficha completa</button><button class="btn gold" data-open-secret-clues>Pistas secretas</button><button class="btn gold" data-nav="classes">${classChosen(p)?'Consultar Classes':'Escolher Classe'}</button></div></section><section class="home-player-themes"><div class="section-heading"><div><span class="section-kicker">TRILHAS PESSOAIS</span><h2>O som deste personagem</h2></div></div><div class="home-theme-grid">${themes.length?themes.map((t,i)=>themeIsAudio(t)?`<button type="button" class="home-theme-card" data-play-theme="${i}"><span>${esc(t.label)}</span><strong>${esc(t.title)}</strong><small>▶ Tocar aqui</small></button>`:`<a class="home-theme-card" href="${esc(safeHref(t.url))}" target="_blank" rel="noopener"><span>${esc(t.label)}</span><strong>${esc(t.title)}</strong><small>Abrir trilha ↗</small></a>`).join(''):'<div class="empty">Este personagem ainda não registrou suas trilhas.</div>'}</div></section>${spotifyHomePanel()}<section class="immersive-cta"><button type="button" class="btn curtain-btn" data-enter-immersive>${CURTAIN_SVG}<span>Entrar em modo sessão</span></button><small>Tela imersiva para jogar: determinação, mochila, diário, anotações, perícias, alma e habilidades.</small></section>`,'home')}
...
function applyConditionOnAdd(e,id){
  ...
  const b=state.sessionBoard;e.conditionTick[id]=b?.active?Math.max(0,Number(b.round)||0):0;
  if(id==='fractured'){
...
function castSpell(p,id,view='sheet'){
  const sp=getSpells().find(x=>x.id===id);if(!sp||!p)return;
...
function playPersonalTheme(url,title){
  const root=document.getElementById('personal-player');if(!root)return;
  const src=safeHref(url,'');if(!src)return;
  let audio=root.querySelector('audio');
  if(!audio){
    root.innerHTML='<div class="personal-player-box"><div class="pp-head"><strong id="ppTitle"></strong><button type="button" id="ppClose" aria-label="Fechar">×</button></div><audio id="ppAudio" controls preload="metadata"></audio></div>';
    audio=root.querySelector('audio');
    root.querySelector('#ppClose').onclick=()=>{audio.pause();audio.removeAttribute('src');audio.load();root.innerHTML=''};
  }
...
async function uploadThemeFile(inp){
  ...
  try{
    ...
    p.musicThemes=Array.isArray(p.musicThemes)?p.musicThemes:[];p.musicThemes.push({label,title,url,kind:'file'});
    save();toast('Música adicionada à trilha do personagem.');render('sheet');
...
function immersivePage(){
  ...
  if(hasMagicClass(p.class))tabs.push(['spells','Magias']);
  ...
  try{
    if(remoteEnabled){
      ...
      const applyIncoming=async(row)=>{
        ...
        if(d.includes('sessionBoard')&&state.session?.role==='player'){
          const me=player(),current=currentTurnParticipant(state.sessionBoard);
          const isMine=!!(me&&current&&current.kind==='player'&&String(current.entityId)===String(me.id));
          if(isMine&&current.id!==lastKnownTurnParticipantId){vibratePhone([120,60,120,60,220]);playTurnAlertSound();toast('É a sua vez!');}
          lastKnownTurnParticipantId=current?current.id:'';
        }
