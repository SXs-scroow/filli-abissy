...
const DEFAULT={version:31,session:null,characterRules:clone(DEFAULT_RULES),classBonuses:clone(DEFAULT_CLASS_BONUSES),music:{type:'',url:'',title:'',kind:'url',mediaId:''},slasherIntroMusic:{url:'',title:'',kind:'url'},sounds:[],players:[basePlayer()],creatures:[],items:DEFAULT_ITEMS,spells:INITIAL_SPELLS,uiIcons:{attrs:{...DEFAULT_ATTR_ICONS},skills:{...DEFAULT_SKILL_ICONS},conditions:{}},backgrounds:{},customContent:{attrs:[],skills:[],conditions:[],deities:[]}};
let memoryStore={};
let bgObjectUrls={};
...
function globalPayload(){
  ...
  return {version:shared.version,players:shared.players,creatures:shared.creatures,items:shared.items,spells:shared.spells,uiIcons:shared.uiIcons,backgrounds:shared.backgrounds,music:shared.music,slasherIntroMusic:shared.slasherIntroMusic,characterRules:shared.characterRules,classBonuses:shared.classBonuses,customContent:shared.customContent};
}
...
function mergeGlobal(remote){
  ...
  const domains=[];
  for(const key of ['players','creatures','items','spells','uiIcons','backgrounds','music','slasherIntroMusic','characterRules','classBonuses','customContent']){
  ...
  if(domains.length){
    ...
    state.backgrounds={...(state.backgrounds||{})};state.slasherIntroMusic=state.slasherIntroMusic||{url:'',title:'',kind:'url'};state.customContent={attrs:[],skills:[],conditions:[],deities:[],...(state.customContent||{})};
    state.characterRules={campaign:{...DEFAULT_RULES.campaign,...(state.characterRules?.campaign||{})},slasher:{...DEFAULT_RULES.slasher,...(state.characterRules?.slasher||{})}};state.classBonuses={...clone(DEFAULT_CLASS_BONUSES),...(state.classBonuses||{})};
...
function hasRemoteSharedData(remote){
  return !!(remote&&typeof remote==='object'&&['players','creatures','items','spells','uiIcons','backgrounds','music','slasherIntroMusic','characterRules','classBonuses','customContent'].some(k=>remote[k]!==undefined));
}
...
function addBag(p){
 const datalistId='bagItemNames';
 const options=state.items.map(i=>`<option value="${esc(i.name)}"></option>`).join('');
 openModal(`<div class="modal" id="bagAddModal"><div class="modal-card"><button class="modal-close" id="closeBagAdd">×</button><h2>Adicionar item à mochila</h2><p class="small muted">Capacidade atual: ${bagCapacity(p)} espaços.</p><div class="field"><label>Pesquisar item</label><input id="bagItemSearch" list="${datalistId}" autocomplete="off" placeholder="Digite o nome do item..."><datalist id="${datalistId}">${options}</datalist></div><div class="field"><label>Quantidade</label><input id="bagItemQty" type="number" min="1" value="1"></div><div class="row" style="justify-content:flex-end"><button class="btn" id="cancelBagAdd">Cancelar</button><button class="btn primary" id="confirmBagAdd">Adicionar</button></div></div></div>`);
 const close=()=>document.getElementById('bagAddModal')?.remove();
 document.getElementById('closeBagAdd').onclick=close;
 document.getElementById('cancelBagAdd').onclick=close;
 document.getElementById('confirmBagAdd').onclick=()=>{
  const q=document.getElementById('bagItemSearch').value.trim().toLowerCase();
  const it=state.items.find(x=>String(x.name||'').trim().toLowerCase()===q);
  if(!it){toast('Item não encontrado. Digite o nome de um item cadastrado.');return}
  const qty=Math.max(1,Number(document.getElementById('bagItemQty').value)||1),weight=inferWeight(it),used=bagWeight(p),cap=bagCapacity(p),ex=p.backpack.find(x=>Number(x.id)===Number(it.id)),current=ex?.qty||0;
  const maxByWeight=Math.floor((cap-used+weight*current)/weight),add=Math.min(it.maxQty-current,qty,maxByWeight);
  if(add<=0){toast('Não há espaço suficiente.');return}
  if(ex)ex.qty=current+add;else p.backpack.push({id:it.id,qty:add});
  save();close();toast(`${add} × ${it.name} adicionado à mochila.`);render('sheet');
 };
}

function slots(e,editable=true){const used=bagWeight(e);return `<div class="row space"><div><h3>Mochila</h3><span class="small ${used>MAX_BAG?'danger-text':'muted'}">${used}/${bagCapacity(e)} espaços usados • peso por unidade: 1–3</span></div>${editable?'<button class="btn small" id="addBagItem">Adicionar item</button>':'<span class="small muted">Somente o Mestre pode alterar</span>'}</div><div class="slot-grid" style="margin-top:12px">${Array.from({length:bagCapacity(e)},(_,i)=>{const b=e.backpack[i],it=b&&item(b.id);return `<div class="slot ${it?'filled':''}">${it?`${editable?`<button data-remove-slot="${i}">×</button>`:''}<div class="slot-icon">${itemIcon(it)}</div><div class="slot-name">${esc(it.name)}</div>${String(it.category||'').toLowerCase().includes('livro')||it.bookContent?`<button class="book-read-btn" data-read-book="${it.id}">Ler</button>`:''}<div class="slot-id">Peso ${inferWeight(it)}</div><div class="slot-qty">Qtd. ${b.qty} • ${inferWeight(it)*b.qty} espaço${inferWeight(it)*b.qty===1?'':'s'}</div>`:'<div class="slot-icon" style="opacity:.15">◇</div><div class="small muted">Espaço vazio</div>'}</div>`}).join('')}</div>`}
